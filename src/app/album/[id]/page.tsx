...
export default async function AlbumPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 bg-gradient-to-b from-blue-900/60 to-transparent px-2 pt-8 pb-6 sm:flex-row sm:items-end sm:gap-6 sm:pt-10">
        <Image
          src={album.cover}
          alt={album.title}
          width={232}
          height={232}
          className="h-40 w-40 shrink-0 rounded-md object-cover shadow-2xl sm:h-56 sm:w-56"
        />
        <div className="min-w-0">
          <p className="text-sm font-semibold text-white">Album</p>
          <h1 className="mt-2 truncate text-3xl font-extrabold text-white sm:text-5xl lg:text-6xl">{album.title}</h1>
